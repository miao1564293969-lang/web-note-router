...
复制内容的前 100 个字符会先在本地进行密钥正则检测。匹配 API Key、Token、Bearer 凭证、密码或
PEM 私钥时，内容会直接写入 `密钥.md`，不会调用 DeepSeek，也不会将密钥发送给大模型。

插件通过 Chrome 的内存型会话存储保存最近 10 条成功记录的 SHA-256。相同内容再次复制时，
插件会提示“重复粘贴”，并在密钥正则和 AI 路由之前终止处理，不重复写入 Markdown。
该记录不会写入磁盘，可跨 Service Worker 休眠保留，并在浏览器完全关闭后清空。
