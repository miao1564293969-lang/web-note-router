...
- 现有标签均不匹配时自动创建新标签和 Markdown 文件；
- 本地识别密钥、Token、密码和私钥，直接写入 `密钥.md`，不发送给大模型；
- 在内存中对最近 10 条内容进行 SHA-256 查重，避免重复写入；
- 支持子目录，例如“学习/AI.md”；
...
- `clipboardRead`：在用户主动复制时读取当次剪贴板中的文字和图片；图片仅写入用户授权的本地目录。
